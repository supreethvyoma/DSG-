...
function AdminDashboard() {
  ...
  const handleBannerFileChange = async (event) => {
    ...
    try {
      ...
      setBannerMessage("Banner image selected. Save Banner to publish it.");
    } catch (err) {
      setBannerMessage(err?.message || "Could not load banner image.");
    } finally {
      event.target.value = "";
    }
  };
  ...
  const saveBannerSettings = async () => {
    ...
    try {
      ...
      setHeroBannerImage(String(res.data?.heroBannerImage || "").trim());
      setBannerMessage("Homepage banner updated.");
    } catch (err) {
      setBannerMessage(err?.response?.data?.message || "Could not save banner.");
    } finally {
        ...
        <section className="card pricing-controls-card">
          <div className="pricing-controls-header">
            <div>
              ...
              <p>Manage the hero banner shown at the top of the home page.</p>
            </div>
          ...
          <div className="banner-admin-grid">
            ...
            <label className="pricing-field">
              <span className="pricing-label">Upload Banner Image</span>
          ...
          {heroBannerImage ? (
            <div className="banner-admin-preview">
              <img src={heroBannerImage} alt="Homepage banner preview" />
            </div>
          ) : (
            <p className="theme-settings-note">
              No banner image is set right now. The home page will begin directly with product sections.
            </p>
          )}

          <div className="pricing-actions-row">
            <button className="pricing-save-btn" onClick={saveBannerSettings} disabled={isSavingBanner}>
              {isSavingBanner ? "Saving..." : "Save Banner"}
            </button>
            <span>Recommended size: wide landscape banner for best results.</span>
          </div>
          {bannerMessage && (
            <p className={`pricing-message ${bannerMessage.includes("updated") || bannerMessage.includes("selected") ? "success" : "error"}`}>
              {bannerMessage}
            </p>
          )}
        </section>
